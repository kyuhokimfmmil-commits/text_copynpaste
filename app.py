...
if run_button:
    if not api_key:
        ...
        try:
            ...
            prompt = '''당신은 법률 및 학술 문서의 띄어쓰기를 복원하는 전문 시스템입니다. 
주어진 텍스트는 PDF에서 복사하여 줄바꿈이 무작위 공백으로 변환된 상태입니다. 
다음 원칙을 100% 엄격하게 준수하십시오.
첫째 원본에 있는 글자는 단 하나도 변경하거나 삭제하거나 추가하지 마십시오. 종결어미나 높임말 및 어투를 임의로 수정하는 행위는 엄격히 금지됩니다. 
둘째 오직 잘못 들어간 공백을 지우거나 필요한 곳에 공백을 넣는 작업만 수행하십시오. 공백을 넣는 작업보다 공백을 지우는 작업이 99%를 차지할 것입니다. 
셋째 문단 구분을 위한 이중 줄바꿈은 반드시 그대로 유지하십시오.
넷째 부가 설명 없이 교정된 최종 텍스트만 출력하십시오.'''
