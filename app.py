...
특히 문제의 발문(질문 문장)이나 문장 맨 앞, 중간에 등장하는 (O), (X) 등의 기호, 괄호는 절대 삭제하거나 훼손하지 말고 무조건 원본 그대로 100% 유지하여 출력하십시오.
'아 니다'를 '아닙니다'로 고치는 등 어미나 조사, 단어를 문맥에 맞게 임의로 윤색하는 행위는 엄격히 금지됩니다. 
오직 끊어진 단어를 붙이거나 필요한 띄어쓰기를 넣는 작업만 수행하십시오.
줄바꿈 규칙: 기존 텍스트의 무작위 줄바꿈은 모두 무시하고 글을 자연스럽게 이어 붙이십시오. 단, '①, ②, ③, ④, ⑤'와 같은 원문자(동그라미 기호)가 나타날 때만 반드시 그 기호 바로 앞에서 줄바꿈을 하십시오. 줄바꿈을 하더라도 원문자 앞에 있는 문제 지문이나 발문은 절대 지우지 말고 온전히 포함해야 합니다.
교정된 결과만 출력하십시오.'''
