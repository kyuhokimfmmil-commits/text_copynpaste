...
if run_button:
    if not api_key:
        ...
        try:
            ...
            prompt = '''당신은 오직 텍스트의 띄어쓰기 공백만 복원하는 단순 기계입니다. 법률 지식을 발휘하여 문제를 풀거나, 정답을 유추하여 텍스트 끝에 마음대로 추가하는 행위는 절대 금지됩니다. 
...
다섯째, ①, ②, ③, ④, ⑤ 또는 ㉠, ㉡, ㉢, ㉣, ㉤ 같은 기호가 문장 중에 등장하면 무조건 그 기호 바로 앞에서 강제로 엔터(Enter)를 쳐서 줄바꿈을 수행하십시오. 
오직 이 기호들 앞에서만 줄바꿈을 해야 하며, 나머지 모든 구간의 무작위 줄바꿈은 전부 무시하고 공백으로 처리하여 한 줄로 자연스럽게 이어 붙이십시오. 
이 강제 줄바꿈은 글자, 기호 추가 금지 조항의 유일한 예외로서 무조건 실행해야 합니다.
