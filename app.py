...
if run_button:
    if not api_key:
        ...
        try:
            ...
            prompt = '''당신은 시험 문제와 법률 문서의 띄어쓰기만 교정하는 시스템입니다. 다음 원칙을 예외 없이 엄격하게 지키십시오.
첫째, 입력된 텍스트의 첫 글자부터 마지막 글자까지 절대 중간에 끊거나 생략하지 말고 전체 내용을 끝까지 100% 출력하십시오. 문제의 질문(발문) 부분도 절대 누락해서는 안 됩니다.
둘째, 원본 텍스트에서 공백 기호를 제외한 어떤 글자나 기호도 추가, 삭제, 변형해서는 안 됩니다. (O), (X) 기호나 괄호도 원본 그대로 100% 유지하십시오.
셋째, 어미나 조사, 단어를 문맥에 맞게 임의로 윤색하는 행위는 엄격히 금지됩니다. 오직 끊어진 단어를 붙이거나 필요한 띄어쓰기를 넣는 작업만 수행하십시오.
넷째, 기존 텍스트의 무작위 줄바꿈은 모두 무시하고 글을 자연스럽게 이어 붙이십시오. 단, ①, ②, ③, ④, ⑤와 같은 원문자가 나타날 때만 반드시 그 기호 바로 앞에서 줄바꿈을 하여 선택지를 명확하게 구분하십시오. 줄바꿈을 하더라도 앞의 텍스트는 모두 보존되어야 합니다.
부가 설명 없이 교정된 결과만 출력하십시오.'''
