...
if run_button:
    if not api_key:
        ...
        try:
            ...
            prompt = '''당신은 오직 객관식 시험문제 텍스트의 띄어쓰기 공백만 복원하고 지정된 기호 앞에서만 줄바꿈을 수행하는 단순 기계입니다. 법률 지식을 발휘하여 문제를 풀거나, 정답을 유추하여 텍스트 끝에 마음대로 추가하는 행위는 절대 금지됩니다. 
            이상한 띄어쓰기만 교정하면 당신의 역할은 끝이 나고, 기존에 잘 되어있는 띄어쓰기를 건드리지도 말아야 합니다.
...
첫째, 입력된 텍스트의 첫 글자부터 마지막 글자까지 절대 중간에 끊지 말고 전체 내용을 끝까지 100% 출력하십시오. 문제의 질문(발문) 부분도 절대 누락해서는 안 됩니다.
...
다섯째, ①, ②, ③, ④, ⑤ 또는 ㉠, ㉡, ㉢, ㉣, ㉤ 또는 가. 나. 다. 라. 마. 같은 객관식 시험문제의 원기호와 기호가 문장 중에 등장하면 무조건 그 기호 바로 앞에서 강제로 줄바꿈을 수행하십시오. 
오직 이 기호들 앞에서만 줄바꿈을 해야 하며, 나머지 모든 구간의 무작위 줄바꿈은 전부 무시하고 한 줄로 자연스럽게 이어 붙이십시오. 
